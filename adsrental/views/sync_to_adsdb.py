...
class SyncToAdsdbView(View):
    def get(self, request):
        leads = Lead.objects.filter(status__in=[Lead.STATUS_IN_PROGRESS, Lead.STATUS_QUALIFIED], is_sync_adsdb=False, fb_email__isnull=False).select_related('raspberry_pi')
        saved_emails = []
        responses = []
